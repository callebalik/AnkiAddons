...
class Texts(NamedTuple):
    ...
    def from_goal(cls, goal: Goal) -> "Texts":
        ...
        if study_days <= 0:
            if new_cards_cnt != 0:
                cards_left_msg = f"It's past due date, but you still need to study <b>{new_cards_cnt}</b> new cards."
            else:
                cards_left_msg = f"It's past due date, and you finished all your cards!"
            return cls (
                cards_left_in_days = cards_left_msg,
                study_new_per = ""
            )
